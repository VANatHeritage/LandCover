...
def plotGeneralChange(yearList, sumTab, outFig):
   ...
   ax1.set_xticks(yearList)  # major ticks at data years
   ax1.set_xticks(list(range(min(yearList), max(yearList)+1, 1)), minor=True)  # minor ticks every year
   ...
   ax2.set_xticks(yearList)   # major ticks at data years
   ax2.set_xticks(list(range(min(yearList), max(yearList)+1, 1)), minor=True)  # minor ticks every year
   ...
   ax1.xaxis.tick_top()  # put ticks on top of top part
   ax1.tick_params(labeltop=False, labelbottom=False)  # don't put tick labels on the top part
   ax2.xaxis.tick_bottom()  # put ticks on bottom of bottom part
...
def main():
   ...
   sumTab = r'C:\David\proc\NLCD_chg\nlcd_2019ed_LandCover_albers_rclsBarrens.gdb\lc_rclsGeneral_changeSummary'
   yearList = [2001, 2004, 2006, 2008, 2011, 2013, 2016, 2019]
   # outFig = r'C:\Users\xch43889\Pictures\pyPlots\lcChange.png'
   outFig = r'D:\git\LandCover\plots\lcChange_rclsGeneral_2019ed.png'
